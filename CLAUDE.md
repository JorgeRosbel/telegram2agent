...
7. `bot.resetSession({ agent?, chatId? })` clears the stored session so the next call starts a fresh conversation — needed when chaining many _independent_ units of work, since one shared session grows without bound and every step resumes the whole history. Persistent per-chat defaults (active agent, model, mode, effort, session IDs for `--resume`) live in `src/state/store.ts` (`StateStore`), backed by a JSON file at `dbPath` (default `<cwd>/.telegram2agent.json`). All three entry points (`ask`, `run`/`runStep`, interactive chat) read _and_ write `store.sessionFor/setSession(chatId, agent)` — that's what keeps them on the same conversation thread.
...
- Enforces its own timeout (`AdapterOptions.timeoutMs`, default 30 min) and kills the process tree on cancel/timeout. It also **reaps the process when the turn settles**: `--input-format stream-json` leaves the CLI waiting on stdin after emitting `result`, so without an explicit EOF every chained step would leak a live `claude` process (this really did OOM a machine). Don't remove the `endStdin()` in `attemptOnce`'s `finish` path.
...
- `ClaudeAdapter.run()` retries automatically (indefinitely, every `usageLimitRetryMs`, default 10 min) when the CLI's own text signals the Claude _plan's_ usage limit was hit (`isUsageLimitError`). The patterns are confirmed against the compiled binary, not public docs: current builds render `You've hit your ${scope} limit · resets <time>` (session/weekly/fast/monthly), older ones said `usage limit reached`; API rate limits count too. A **spend** limit is deliberately excluded — it needs a human to raise the cap, so sleeping on it would hang the run forever. While waiting, the owning `Task`'s timeout is paused (`pauseTimeout`/`resumeTimeout` via `onUsageLimitWait`/`onUsageLimitResume`) so `taskTimeoutMs` measures agent work, not the wait. Any other failure (auth, timeout, agent bug) fails immediately as before; the wait is cancellable via the same `RunHandle.cancel()`/`task.cancel()` path. `onUsageLimitWait` fires once per wait so callers (interactive chat, `launchTask`) can notify the chat without spamming it every retry.
